...
@Autonomous(name = "Red-Goal-Preload")
@Configurable // Panels
public class ApocGoalPreloadRed extends OpMode {
    private TelemetryManager panelsTelemetry; // Panels Telemetry instance
    public Follower follower; // Pedro Pathing follower instance
    private int pathState=0; // Current autonomous path state (state machine)
    private ApocGoalPreloadRed.Paths paths; // Paths defined in the Paths class
    ...
    public void init() {
        ...
        follower.setStartingPose(new Pose(121, 121, Math.toRadians(216)));
            ...
            StartChain = follower.pathBuilder()
                    .addPath(
                            new BezierLine(
                                    ...
                                    new Pose(80.000, 115.000)
                            )
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(216), Math.toRadians(205))
                    .build();
            FinishChain = follower.pathBuilder()
                    .addPath(
                            new BezierLine(
                                    new Pose(80.000, 115.000),
                                    new Pose(127.000, 100.000)
                            )
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(205), Math.toRadians(0))
                    .build();
        }
    }
